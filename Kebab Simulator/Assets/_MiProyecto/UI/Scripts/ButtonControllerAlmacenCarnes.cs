...
public class ButtonControllerAlmacenCarnes : MonoBehaviour
{
    ...
    {
        headerNombreIngrediente.transform.GetChild(0).GetComponent<Text>().text = ingredienteCarne.nombre;
        textoCantidadAlmacen.text = ingredienteCarne.unidadesAlmacen.ToString();
        setColorCantidadAlmacen();
        textoCantidadCocina.text = "Cocina: " + prefabIngrediente.GetComponent<AñadirCarne>().cantidad + "/" + ingredienteCarne.capacidadMaxIngrediente.ToString();
    ...
    {
        if(ingredienteCarne.unidadesAlmacen == 0)
        {
